...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "printf '%s\\n' '${var.image_version}' | sudo tee /opt/frog/WORKER_RELEASE_SHA >/dev/null",
      "printf '%s\\n' '${local.image_name}' | sudo tee /opt/frog/WORKER_IMAGE_NAME >/dev/null",
      "sudo chown -R frogworker:frogworker /opt/frog/app",
      "sudo chown frogworker:frogworker /opt/frog/WORKER_RELEASE_SHA /opt/frog/WORKER_IMAGE_NAME",
